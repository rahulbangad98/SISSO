...
# #columns_to_drop = ['r_s_A (AA)', 'r_s_B (AA)','r_pi (AA)']
#columns_to_drop = ['r_s_a (aa)', 'r_s_b (aa)']
        ...
        for iteration_dir in os.listdir(holdout_path):
            ...
            if os.path.isdir(iteration_path):
                ...
                if os.path.exists(sisso_file_path):
                    ...
                    print(additional_file_source_path)
                    if os.path.exists(additional_file_source_path):
                        ...
                        try:
                            #num_columns_to_drop = int(input("Enter the number of columns to drop: "))
                            num_columns_to_drop = 19
                            if num_columns_to_drop < 1 or num_columns_to_drop >= len(df.columns):
                        ...
                        #ensuring atleast 1 column should be present
                        #num_columns_to_drop = random.randint(1, len(df.columns) - 1)  
                        columns_to_drop = random.sample(list(df.columns), num_columns_to_drop)
...
# def process_file_set(it_number, base_path, columns_to_drop):
#     # Generate the file paths
#     model_files = [os.path.join(base_path, f"models/Holdout_{it_number}/Iteration_{i}/models/train_dim_4_model_0.dat") for i in range(1, 11)]
#     file_path = os.path.join(base_path, f"models/Holdout_{it_number}/holdout_set_{it_number}.csv")
#     print(file_path)
...
#     # Load the test data and drop specified columns
#     data_predict = pd.read_csv(file_path)
#     data_predict.drop(columns=columns_to_drop, inplace=True, errors='ignore')
...
#     data_predict['Std Deviation'] = std_predictions
    ...
    #print(f"All Prediction Errors for iteration {it_number}:")
    #print(data_predict[['# Material', 'EA_A', 'Average Predicted EA_A', 'Error', 'Large Error']])
...
#sisso.json me jo csv hai usmese kuch coluns drop krne hai phir model create krna hai phir test


#check for the avg
#print all the material except the outliers